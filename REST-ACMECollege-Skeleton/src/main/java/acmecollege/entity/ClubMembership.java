...
@NamedQuery(name = MyQueriesConstants.CLUB_MEMBERSHIP_FIND_ALL, query = "SELECT cm FROM ClubMembership cm left join fetch cm.club left join fetch cm.card")
@NamedQuery(name = MyQueriesConstants.CLUB_MEMBERSHIP_FIND_BY_ID, query = "SELECT cm FROM ClubMembership cm left join fetch cm.club left join fetch cm.card where cm.id = :param1")
