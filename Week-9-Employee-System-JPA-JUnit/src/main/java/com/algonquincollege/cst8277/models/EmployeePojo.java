...
public class EmployeePojo extends PojoBase implements Serializable {
    ...
    //I'm not using CascadeType.All (skipping CascadeType.REMOVE): what if two employees
    //live at the same address and 1 leaves the company but the other does not?
    ...
    // these overrides are here just-in-case over on EmployeeTask the @Basic's dont have proper @Column annotations
    @AttributeOverrides({
        @AttributeOverride(name = "description", column = @Column(name = "TASK_DESCRIPTION")),
        @AttributeOverride(name = "taskStart", column = @Column(name = "TASK_START")),
        @AttributeOverride(name = "taskEnd", column = @Column(name = "TASK_END")),
        @AttributeOverride(name = "taskDone", column = @Column(name = "TASK_DONE"))
    })
    ...
    //this time I AM using CascadeType.ALL: if an employee leaves the company, remove their
    //phones ... because in some countries a phone number might be PPI 'Protected Personal Information'
    //and legislation might mandate not holding on to it.
    ...
    public void addProject(ProjectPojo p) {
        if (p != null) {
            getProjects().add(p);
            p.addEmployee(this);
        }
